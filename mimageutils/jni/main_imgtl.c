...
int main(int argc, char *argv[])
{
	...
	while (idx < argc){
		...
		if (arg_rotate != NULL)
		{
			if (strcmp(arg_rotate, "l") == 0)
			{
				//gen resized image buffer
				unsigned char* src_rgb = rgb;
				
				//gen new rgb
				rgb = aspire_mao_rgb_rotate_left(src_rgb, width, height, pixel_bytes);
				int temp = width;
				width = height;
				height = temp;
			...
			else if (strcmp(arg_rotate, "r") == 0)
			{
				//gen resized image buffer
				unsigned char* src_rgb = rgb;
				
				//gen new rgb
				rgb = aspire_mao_rgb_rotate_right(src_rgb, width, height, pixel_bytes);
				int temp = width;
				width = height;
				height = temp;
